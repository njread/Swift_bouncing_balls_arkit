...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        guard let surfaceNode = scene.rootNode.childNode(withName: "floor", recursively: true) else {return}
            for surfaceNode in surfaceNode.childNodes{
                let physicsBody = SCNPhysicsBody(type: .dynamic, shape: nil)
                physicsBody.applyForce(forceDirection, asImpulse: true)
                surfaceNode.physicsBody = physicsBody
            }
